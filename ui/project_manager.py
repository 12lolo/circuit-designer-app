...
class ProjectManager:
    ...
    def generate_thumbnail(self, scene: QGraphicsScene, size: int = 200, grid_rect=None) -> str:
        ...
        if grid_rect:
            ...
            print(f"[DEBUG] Using grid_rect: {grid_rect}")
            print(f"[DEBUG] Render rect: x={render_rect.x()}, y={render_rect.y()}, w={render_rect.width()}, h={render_rect.height()}")
        else:
            # Fallback to items bounding rect
            render_rect = scene.itemsBoundingRect()
            print(f"[DEBUG] No grid_rect provided, using itemsBoundingRect: x={render_rect.x()}, y={render_rect.y()}, w={render_rect.width()}, h={render_rect.height()}")

            # If empty, use scene rect
            if render_rect.isEmpty() or (render_rect.width() <= 0 or render_rect.height() <= 0):
                render_rect = scene.sceneRect()
                print(f"[DEBUG] Items rect empty, using sceneRect: x={render_rect.x()}, y={render_rect.y()}, w={render_rect.width()}, h={render_rect.height()}")
